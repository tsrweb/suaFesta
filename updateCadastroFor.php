...
if($imgLogo != NULL) {
  ...
  if (move_uploaded_file($imgLogo['tmp_name'], $nomeFinal)) {
    ...
    $link->query("UPDATE tb_usuario SET img_logo = '$mysqlImg' WHERE id_usuario = '$idUsuario' ");
  ...
  if (move_uploaded_file($foto1['tmp_name'], $nomeFinal)) {
    ...
    $link->query("UPDATE tb_usuario SET img_foto1 = '$mysqlImg1' WHERE id_usuario = '$idUsuario' ");
  ...
  if (move_uploaded_file($foto2['tmp_name'], $nomeFinal)) {
    ...
    $link->query("UPDATE tb_usuario SET img_foto2 = '$mysqlImg2' WHERE id_usuario = '$idUsuario' ");
  ...
  if (move_uploaded_file($foto3['tmp_name'], $nomeFinal)) {
    ...
    $link->query("UPDATE tb_usuario SET img_foto3 = '$mysqlImg3' WHERE id_usuario = '$idUsuario' ");
  ...
  if (move_uploaded_file($foto4['tmp_name'], $nomeFinal)) {
    ...
    $link->query("UPDATE tb_usuario SET img_foto4 = '$mysqlImg4' WHERE id_usuario = '$idUsuario' ");
...
if ($_POST && isset($_POST['ativo'])){
	...
	$con2 = $link->query("DELETE FROM tb_servicosUsuarios WHERE id_usuario = '$idUsuario' ");
...
<html lang="PT-BR">
  ...
  <body>
    ...
    function showPage() {
    window.location=("visualizarFornecedor.php?idFesta=sf&idFor=<?=$idUsuario?>");
    }
